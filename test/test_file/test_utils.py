...
#color=0 when frame is BGR scale, color=cv2.COLOR_BGR2GRAY for other situations
def show_event_frame(timestage, color, dataset):
    cv2.namedWindow("Event Stream", cv2.WINDOW_AUTOSIZE)
    iter_size = len(os.listdir("test/dataset/" + dataset + "/RGB_frame"))
    for iter in range(1, iter_size + 1):
        print("Iter: ", iter)
        events = load_event_mat(iter, dataset)
        frame = load_frame_png(iter, color, dataset)
        cv2.imshow("Event Stream", frame)
        cv2.waitKey(1)

        for time in (np.array(range(1, timestage + 1)) / (timestage / 0.02)):
            ...
            for i, event in enumerate(events):
                if(event[3] > time):
                    np.delete(events, np.s_[:i], 0)
                    break
                frame_x = np.uint16((event[0] - 1) * 8)
                frame_y = np.uint16((event[1] - 1) * 8)
                cv2.circle(frame, (frame_x, frame_y), 3,
                        (255, 0, 0) if event[2] == 1 else (0, 0, 255))
                
            cv2.putText(frame, dataset + ", Iter: " + str(iter), (200, 200), cv2.FONT_HERSHEY_COMPLEX,
                        0.7, (0, 0, 255), 2)
            cv2.imshow("Event Stream", frame)
            cv2.waitKey(1)
    cv2.waitKey(0)
...
if __name__ == '__main__':
    # show_event_frame(20, cv2.COLOR_BGR2GRAY, "Indoor4")
    # frame = load_frame_png(3, cv2.COLOR_BGR2GRAY, "Indoor4")
    # show_cell_pics(frame)
